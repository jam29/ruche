...
exports.init = function(socket,sockets,callback) {
  connection.query("SELECT id,id_groupe,mot,poids from nuage where phase=4 order by mot",function(err,rows,fields) { 
    if (err) throw err; 
    callback(null, { 
      groupe: {
        template: "saisie_nuage.ejs",
        data: "saisie_nuage4.json",
        cible:"container"
      },
       moderateur: {
        template: "collecte_nuage.ejs",
        data: "collecte_nuage4.json",
        cible: "container",
        bdd:rows
      },
      animateur: {
...
exports.storeResponse = function(socket,sockets,data) {
  ...
  //---- sinon groupe  
    ...
    connection.query("SELECT id,mot,poids FROM nuage WHERE phase = 4 AND mot =" + mot +" LIMIT 1", function(err,rows,fields) {
    console.log(rows);
	if (rows.length == 0 ) {
    		connection.query("INSERT INTO nuage VALUES ( '', "+ data.idgrp +"," + mot + ",1 ,4,CURRENT_TIMESTAMP )", function(err) { 
      			if (err) { throw err; }
