var db = require('./persistence') ;
var connection = db.connection ;

exports.init = function(socket,sockets,callback) {
  connection.query("SELECT reponse,id_groupe,commentaire FROM reponses WHERE sequence ='009' AND phase='001'" ,function(err,rows,fields) {
  if (err) throw err;   
  callback(null, { 
      groupe: {
        seq:'009',
        phas:'001',
        template: "questions_classement2.ejs",
        data: "questions_classement_9.json",
        cible:"container" ,
        bdd:rows
      },
       moderateur: {
        template: "affiche_reponse.ejs",
        data: "questions_classement_9.json",
        cible:"container" ,
        bdd:rows
      },
      animateur: {
        template: "mode_emploi.ejs",
        data: "mode_emploi_sortable.json",
        cible:"container"
      },
      diffuseur: {
        template: "mode_emploi.ejs",
        data: "mode_emploi_sortable.json",
        cible:"container" 
      }
   })
 })
}

exports.storeResponse = function(socket,sockets,data) {

        var commentaire = connection.escape(data.commentaire) ;	
        var query = "SELECT id FROM reponses WHERE sequence ='" + data.seq + "' AND phase ='"+data.phas + "' AND id_groupe=" + data.idgrp ;
       
          connection.query(query,  function(err,rows2,fields) {
            if (err) throw err;
            if ( rows2.length == 0 ) 
              connection.query("INSERT INTO reponses VALUES ( '','" + data.seq + "','"+ data.phas+"',"+data.idgrp+",'"+data.reponse+"', "+commentaire+" )", function(err) { if (err) throw err; });
            else if ( data.type != "new") {
              connection.query("UPDATE reponses set reponse='"+data.reponse+"' , commentaire = "+commentaire+" WHERE id = "+rows2[0].id, function(err) 
 	               { if (err) throw err; });
			socket.emit('message',{ message:'Votre classement est enregistré' } );
              }
process.emit('moderateur:refresh');
            })
}
