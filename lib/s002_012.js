...
exports.init = function(socket,sockets,callback) {
  connection.query("SELECT mot AS text, poids AS weight FROM nuage WHERE phase=4 AND poids > 0 ",function(err,rows,fields) { 
  if (err) throw err; 
  callback(null, { timeout: 1500,
      groupe: {
      template: "regarde_diffuseur.ejs",
        data: "regarde_diffuseur4.json",
        cible:"container"
      },
       moderateur: {
        template: "affiche_nuage.ejs",
        data: "affiche_nuage4.json",
        cible:"fonction" ,
        bdd:rows
      },
      animateur: {
