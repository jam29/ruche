...
exports.init = function(socket,sockets,callback) {
  connection.query("SELECT sequence,phase,id_groupe,reponse,commentaire FROM reponses WHERE sequence='003' AND phase='001' ",function(err,rows,fields) {
  if (err) throw err; 
  callback(null, {
      groupe: {
        seq:'003',
