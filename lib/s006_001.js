...
exports.init = function(socket,sockets,callback) {
  connection.query("SELECT reponse,id_groupe,commentaire FROM reponses WHERE sequence ='006' AND phase='001'" ,function(err,rows,fields) {
  if (err) throw err;   
  console.log(rows);
  callback(null, { 
      groupe: {
...
exports.storeResponse = function(socket,sockets,data) {
 ...
 socket.emit('message',{ message:"selection enregistrée" } );
process.emit('moderateur:refresh');
process.emit('animateur:refresh');
            })
}
