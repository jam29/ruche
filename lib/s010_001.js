...
exports.init = function(socket,sockets,callback) {
  connection.query("SELECT * FROM mail WHERE ruche=10 ORDER BY date",function(err,rows,fields) { 

    callback(null, { 
      groupe: {
        template: "question_salle.ejs",
        data: "question10.json",
        cible:"container"
      },
       moderateur: {
        template: "collecte_questions_nogo.ejs",
        data: "question10.json",
        cible: "container",
        bdd:rows
      },
...
exports.storeResponse = function(socket,sockets,data) {
 ...
 if ( data.question ) {
   ...
   connection.query("INSERT INTO  mail VALUES ('',"+data.idgrp+",NOW(),10,'ruche auvergne4bis : groupe "+data.idgrp+"',"+question+","+question+",0,0,0)", function (err) { if (err) throw err })
   socket.emit('message',{message:'question enregistrée'});
 }
